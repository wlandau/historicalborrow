...
\end{aligned}
$$
Our $w$ from before is thus $p(M_\text{pool} | y)$.
...
\end{aligned}
$$
An exploration of $p(\mu | y, w)$ over a range of values of $w$ guides intuition. In other words, how large (or small) would $w$ have to be in order to trigger a probabilistic decision rule (say, $P(\mu > x | y, w)$)?
