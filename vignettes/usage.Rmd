...
There is also an `hb_summary_weighted()` function that summarizes a mixture between the posteriors of the independent and pooled models (see the methods vignette for the specific model averaging techniques). The output is the same as the `hb_summary()` function except there is a new `weights` column which accepts a vector of weights on the pooled model, and there is an entire row group for every scalar element of `weights`.

```{r}
summary_weighted <- hb_summary_weighted(
  mcmc_pool = mcmc_pool,
...
You may choose to compute one or more of the weights with the `hb_weight_bma()` function, which in turn requires marginal likelihoods given by `hb_mll_pool()` and `hb_mll_independent()`. If you do, please be sure to supply a sufficient number of posterior samples to the marginal log likelihood functions to ensure convergence.
...
To explore weighted averages among the independent and pooled models, e.g. in a tipping point analysis, use the `hb_plot_weighted()` function. You may choose to highlight one or more results for a given set of weights using the `highlight_weights` argument.

```{r}
hb_plot_weighted(
  summary = summary_weighted,
  outcome = "response",
