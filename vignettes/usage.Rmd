...
The `hb_metrics()` function shows legacy/superseded borrowing metrics like the mean shift ratio (no longer recommended as a borrowing metric) and variance shift ratio which require input from benchmark models. The mean shift ratio and variance shift ratio are defined in the methods vignette. Below, we compute these metrics on the hierarchical model.
